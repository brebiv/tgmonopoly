...
# uv run manage.py migrate && uv run manage.py populate_database || echo "Ignoring"
uv run manage.py reset_db <<< "yes" && uv run manage.py migrate && uv run manage.py populate_database
